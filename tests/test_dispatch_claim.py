...
class TestConcurrentTransition:
    async def test_concurrent_queued_to_running_has_exactly_one_winner(
        self, persistence
    ):
        ...
        assert len(rejected) == 3, (
            "exactly one transition into RUNNING may succeed; "
            f"{4 - len(rejected)} succeeded"
        )
...
class TestClaimNextQueuedJob:
    ...
    async def test_concurrent_claims_of_one_job_yield_one_winner(self, persistence):
        ...
        results = await asyncio.gather(
            *(persistence.claim_next_queued_job() for _ in range(20))
        )

        winners = [r for r in results if r is not None]
        assert winners == ["only_one"], f"claimed {len(winners)} times, expected 1"

    async def test_concurrent_claims_never_hand_out_the_same_job_twice(
        self, persistence
    ):
        for i in range(5):
