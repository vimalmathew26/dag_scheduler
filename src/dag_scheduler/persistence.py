...
class InvalidTransitionError(Exception):
    ...
    def __init__(self, job_name: str, from_state: JobState, to_state: JobState) -> None:
...
class Persistence:
    ...
    async def setup(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_job_runs_state ON job_runs(state)")
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_job_logs_run_id ON job_logs(job_run_id, id)"
            )
    ...
    async def set_job_attempt(self, name: str, attempt: int) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE jobs SET current_attempt = ? WHERE name = ?", (attempt, name))
            await db.commit()
    ...
    async def claim_next_queued_job(self, eligible: set[str] | None = None) -> str | None:
    ...
    async def block_queued_jobs_without_definitions(self, eligible: set[str]) -> list[str]:
        ...
        for name in orphaned:
            # If it moved underneath us the next pass will pick it up.
    ...
    async def count_runs_in_state(self, state: JobState) -> int:
        ...
        async with (
            aiosqlite.connect(self.db_path) as db,
            db.execute("SELECT COUNT(*) FROM job_runs WHERE state = ?", (state.value,)) as cursor,
        ):
            row = await cursor.fetchone()
            return int(row[0]) if row else 0
    ...
    async def record_cancelled_run(self, job_name: str) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                ...
                VALUES (?, ?, ?, NULL, ?, NULL, ?)
                """,
                (str(uuid.uuid4()), job_name, JobState.CANCELLED.value, end_time, attempt),
            )
            await db.commit()

    async def get_job_state(self, name: str) -> JobState | None:
        """Current state of a job, or None if it is not in the database."""
        async with (
            aiosqlite.connect(self.db_path) as db,
            db.execute("SELECT state FROM jobs WHERE name = ?", (name,)) as cursor,
        ):
            row = await cursor.fetchone()
    ...
    async def finalize_run(self, run_id: str, state: "JobState", exit_code: int | None) -> str:
