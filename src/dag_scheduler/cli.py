...
from .config import API_HOST, API_PORT, JOBS_DIR

DEFAULT_API_URL = os.environ.get("DAG_SCHEDULER_API_URL", f"http://{API_HOST}:{API_PORT}")
...
@click.option(
    "--api-url",
    default=None,
    help="Base URL of the daemon API. Defaults to DAG_SCHEDULER_API_URL or http://127.0.0.1:8000.",
)
def cli(api_url: str | None) -> None:
    """DAG Scheduler CLI"""
    global API_BASE_URL
    if api_url:
...
@cli.command()
@click.option(
    "--jobs-dir", type=click.Path(), default=None, help="Directory to read job definitions from."
)
def daemon(jobs_dir: str | None) -> None:
...
def runs(name: str) -> None:
    ...
    try:
        ...
        for run in runs:
            ...
            click.echo(
                f"  {run_id_short}  attempt={attempt}  state={state}  exit={exit_code}  start={start_time}"
            )
    except httpx.RequestError:
        click.echo("Daemon not running (connection refused)", err=True)
        sys.exit(1)
...
def stats() -> None:
    ...
    try:
        ...
        click.echo(
            f"  Average Duration: {stats_data.get('avg_duration_seconds', 'N/A'):.2f} seconds"
        )
